...
namespace ParserKinopoisk
{
    class WebConnect
    {
        ...
        async Task<string> GetResponse(string param_string)
        {
            try
            {
                using (var httpclient = new HttpClient { BaseAddress = baseAddress })
                {
                    ...
                    httpclient.DefaultRequestHeaders.Add("Accept-Language", "ru-Ru,ru;q=0.5");
                    httpclient.DefaultRequestHeaders.Add("Accept-Charset", "Windows-1252,utf-8;q=0.7,*;q=0.7");
        ...
        public async Task<List<FilmShot>> GetImageList(int film_id)
        {
            string response_data = await GetResponse($"{film_id}/stills/");
